...
3.  Check your global __TypeScript__ version:  
    ...
    - `tsc` is needed to generate the __bad-checks.d.ts__ type declaration
...
`git clone git@github.com:0bdx/bad-checks.git && cd bad-checks`

Install the two dev-dependencies:  
`npm i`  
@0bdx/build-helpers 0.0.4, 1 package, 22 kB for 6 items.  
rollup 3.17.2, 2 packages, 2.6 MB for 32 items.  

Open the `bad-checks` repo in VS Code:  
`code .`
...
Build __bad-checks.js__ and __bad-checks.d.ts__:  
`npm run build:prod`  
`npm run build:types`

Run all tests on the built __bad-checks.js__ file:  
`npm run preflight:test`

Check that __bad-checks.js__ uses all types correctly:  
...
Publish to [npmjs.com/package/@0bdx/bad-checks](
https://www.npmjs.com/package/@0bdx/bad-checks):  
`npm publish`
...
```sh
touch bad-checks.d.ts bad-checks.js
```
...
sed -ix 's/§0/bad-checks./g' *e.json
...
8. Specify the minimum supported Node.js version:  
   `sed -ix 's/§3/"engines": { "node": ">= 14.0.0" }/' *e.json`
9. The first script generates the main file, __bad-checks.js__, and then  
   the second script generates the type declarations, __bad-checks.d.ts__.  
   ...
   `sed -ix 's/§c/"§Z": "for s in {§A,§B};do npm run §Z:$s;done"/' *e.json`  
   `sed -ix 's/§A/prod/g;s/§B/types/g;s/§Z/build/g;' *e.json`  
10. The fourth script runs unit tests on the main file, __bad-checks.js__,  
...
15. Install three dev-dependencies:  
    ...
    `npm install rollup -D` 3.17.2, 2 packages, 2.6 MB for 32 items  
